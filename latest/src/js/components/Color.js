export class Color {
    ...
    static toSixDigitHex(validHex) {
        let prefix = validHex[0]
        let suffix = validHex.slice(1)
        let extended = suffix.length === 3 ? suffix.split('').map(v => v + v).join('') : suffix
    ...
    static convertNamedColor(str){
        return str.startsWith('#') ? str : isValidNamedColor(str)
    }
    static namedColors = {
            'none':'none',
            aliceblue: "#F0F8FF",
    ...
    static isValidNamedColor(str){
        return Color.namedColors[str]
    }
    static isValidHex(str) {
            // Regular expression to match a valid hexadecimal color code
            const hexRegex = /^#?([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/;
            // Check if the string matches the regular expression
            if (this.isValidNamedColor(str)) return true;
