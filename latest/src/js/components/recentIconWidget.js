    ...
    let widgetHTML = `
      <div class="recent-collection" name=${collection.name}>
        ...
        <div class="control">
          ...
          ${collection.synced ? `<div class="isSyncedControl"><div class="opt opt-view" href="/browse/${collection.name}" data-link="">view</div><div class="opt opt-remove">remove</div></div>` : '<div class="opt option-accept">Upload</div><div class="opt option-ignore">Ignore</div>'}
        </div>
      </div>
    `
    widget.innerHTML = widgetHTML;
    $('.collection-info',widget).addEventListener('click',() => this.notify('preview',collection))
    $('.control',widget).addEventListener('click',(event) => {
      const upload = event.target.closest('.opt.option-accept')
      if (upload){
        console.log('uploading.....')
        this.notify('upload',collection,widget)
      }
    })
    return widget;
  }
