...
module.exports.from = function from(since) {
    ...
    const ago = {
        ...
        millisecondsAgo: msAgo,
        leaps: leapSince,
        string: undefined,
    };
    const findPlural = (agoDate,agoContext) => {
       return agoDate === 1 ? agoContext.slice(0,agoContext.length - 1) : agoContext
    }
    if (ago.yearsAgo >= 1) {
        if (ago.months >= 1) 
        ...
        let rounded = ago.weeksAgo === 3 && ago.daysAgo > 21 ? 4 : 3
        ago.string = `${rounded} ${findPlural(rounded,'Weeks')} ${context}`
    }

    else if (ago.daysAgo < 14 && ago.daysAgo > 2) {
        ago.string = `${ago.daysAgo} ${findPlural(ago.daysAgo,'Days')} ${context}`
    }
    else if (ago.hoursAgo <= 48 && ago.hoursAgo >= 1) {
        let rounded = (ago.hoursAgo < 2 && ago.hoursAgo >=1) ? 1 : ago.hoursAgo
        ago.string = `${rounded} ${findPlural(rounded,'Hours')} ${context}`
    }
